...
@WebServlet(urlPatterns = "/video.do", initParams = {@WebInitParam(name = "videoPath",
						value = "D:/contents")})
public class VideoStreamingServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		if(video==null || video.trim().length()==0) {//image라는 파라미터가 안넘어온것
			//null,white space check
			resp.sendError(HttpServletResponse.SC_BAD_REQUEST);
			return;
		}
		File videoFile = new File(folder,video); //parent,child   parent 객체 폴더의 child 라는 파일에 대한 File 객체를 생성한다.
		if(!videoFile.exists()) {
			//이미지가 없을때
			//응답상태코드 전송
		...
		String mime = getServletContext().getMimeType(video);//여러가지 확장자를 고려해서..
